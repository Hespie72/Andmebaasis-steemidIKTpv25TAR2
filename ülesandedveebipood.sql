...
iseseisvalt:
tabel Categoryes - loo protseduurid, millega saab lisada, uuendada, kustuda

CREATE PROCEDURE lisaCategori
@categoriNimi varchar(30)
as
begin
select * from categories;
insert into categories(category_name) values (@categoriNimi);
select * from categories;
end;
EXEC lisaCategori 'kindad'
--protseduri mis kustuta from categories
CREATE PROCEDURE kustutaCategori
@id int
AS
BEGIN
SELECT * FROM categories;
...
END;
--KUTSE
EXEC kustutaCategori 3;
EXEC kustutaCategori @id=3;

--procedure mis uuendab UPDATE brandinimi id järgi
CREATE PROCEDURE uuendaCategory
@id int,
@uus_categorydNimi varchar(30)
as
begin
select * from categories;
update categories set category_name=@uus_categorydNimi
